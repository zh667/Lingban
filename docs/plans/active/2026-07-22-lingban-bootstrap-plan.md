...
| 项 | 归属 | 理由 |
| --- | --- | --- |
| 跨工单环路检测 + Trace 环标记(#3 余下) | M2 | 需要应用层事务内图检查,随报工工具一起设计 |
| 复合租户外键 (TenantId, Id)(#5 完整) | 多租户真实启用前 | 大迁移;当前单一 default 租户,域内校验已挡住主要路径 |
| 消耗上报幂等键(#8) | M2/M3 | API/工具层尚不存在,幂等键设计跟随调用方形态 |
| regex 守卫升级 Roslyn analyzer(#11 完整) | 守卫被真实绕过一次时 | 生长纪律:等真实事故,不预先镀金 |
| Identity 租户 membership / Todo 移除(#12) | Todo 随 M2 移除;Identity 在鉴权接入时 | 模板遗留,当前无多租户运行 |
| OEE 用班次区间集合而非首尾包络(#9 余下) | M2 OEE 工具移植 | GetProductionDayBoundsUtc 语义已注明是包络 |
| Complete 前置校验(消耗>0、产出=报工)(#7 余下) | M2 报工流程 | 需先定义无料工单等合法形态,不默认放行也不误杀 |
